...
Examining all studies together, a test for heterogeneity by type of experiment (field or survey) reveals that up to 66\% of the total heterogeneity across studies can be accounted for by a dummy variable for type of experiment (0 = survey, 1 = field) in the model. This dummy variable has a significant association with the effectiveness of the information treatment at the 1\% significance level. In fact, the point estimate of this dummy variable is equal to 0.32, while the overall estimate across studies is -0.33,\footnote{Using a mixed effects model with a field experiment moderator.} implying that the predicted treatment effect across experiments is not significantly different from zero when an indicator for type of experiment is included in the model. 
